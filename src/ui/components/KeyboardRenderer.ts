...
import { KeyboardControls } from "../../utils/keyboardMap"
...
export class KeyboardRenderer {
   ...
   */
  private getKeyChar(keyCode: number, p5: P5): string {
    // Special key mapping
  ...
  private renderKey(p5: P5, x: number, y: number, keyCode: number, isPressed: boolean): void {
    const keyChar = this.getKeyChar(keyCode, p5)
  ...
  renderControlsText(p5: P5, x: number, y: number): void {
    ...
    const p1LeftKey = this.getKeyChar(player1.left, p5)
    const p1RightKey = this.getKeyChar(player1.right, p5)
    const p1JumpKey = this.getKeyChar(player1.jump, p5)
    const p1AttackKey = this.getKeyChar(player1.attack, p5)
    
    const p1Text = `P1 Move: Left ${p1LeftKey} Right ${p1RightKey} | Jump: ${p1JumpKey} | Attack: ${p1AttackKey}`
    p5.text(p1Text, x, y)
    
    // Player 2 control hints
    const p2LeftKey = this.getKeyChar(player2.left, p5)
    const p2RightKey = this.getKeyChar(player2.right, p5)
    const p2JumpKey = this.getKeyChar(player2.jump, p5)
    const p2AttackKey = this.getKeyChar(player2.attack, p5)
